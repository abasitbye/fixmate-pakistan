...
import { refundDecisionSchema } from "@/lib/marketplace/payments/schemas";
import { decideRefund } from "@/lib/marketplace/payments/service";

export async function POST(
  request: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  const auth = await getPaymentContext("Sign in to decide a refund.");
  ...
  if (!parsed.success)
    return apiError(
      400,
      "INVALID_REFUND_DECISION",
      "Document the refund decision.",
    );
  const { data, error } = await decideRefund(
    auth.context,
    (await params).id,
