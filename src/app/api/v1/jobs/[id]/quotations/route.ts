...
import { listJobQuotations, saveQuotation } from "@/lib/marketplace/execution/service";
import { isMarketplaceFeatureEnabled } from "@/lib/marketplace/feature-flags";

export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  const context = await getAuthenticatedContext();
  if (!context) return apiError(401, "UNAUTHENTICATED", "Sign in to view quotations.");
  ...
  const { data, error } = await listJobQuotations(context, (await params).id);
  if (error) return apiError(404, "JOB_NOT_FOUND", "The job or quotations were not found.");
  return apiSuccess({ quotations: data });
}

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const context = await getAuthenticatedContext();
  if (!context) return apiError(401, "UNAUTHENTICATED", "Sign in to prepare a quotation.");
  ...
  const { data, error } = await saveQuotation(context, (await params).id, parsed.data);
  if (error || !data) return executionCommandError(error);
  return apiSuccess({ quotation: data }, { status: 201 });
}
